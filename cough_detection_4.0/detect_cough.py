    ...
    with sd.InputStream(channels=1, callback=lambda indata, frames_list, time, status: callback(indata, frames, time, status),
        ...
        while not stop_event.is_set():
            ...
            if len(frames) > 0:
                ...
                if convert_prob >= THRESHOLD:
                    print(f"Cough detected on Microphone {device}, Severity: {convert_prob}%")
                    ...
                    print(f"No cough detected on Microphone {device}")
def main():
    model = pickle.load(open(os.path.join('./models', 'cough_classifier'), 'rb'))
    scaler = pickle.load(open(os.path.join('./models', 'cough_classification_scaler'), 'rb'))
    ...
    thread_1 = threading.Thread(target=record_cough_thread, args=(microphone_ID_1, duration, fs, model, scaler, stop_event))
    thread_2 = threading.Thread(target=record_cough_thread, args=(microphone_ID_2, duration, fs, model, scaler, stop_event))
